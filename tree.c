...
//
// Returns 0 on success, -1 on error.
static int build_tree(Index *index, const char *prefix, ObjectID *id_out) {
    Tree tree;
    tree.count = 0;
    ...
    for (int i = 0; i < index->count; i++) {
        ...
        // Check if this entry belongs to this level
        if (strncmp(path, prefix, prefix_len) != 0)
            continue;

        const char *rest = path + prefix_len;

        // Skip deeper levels (handled separately)
        ...
        if (slash) {
            ...
            ObjectID subtree_id;
            if (build_tree(index, new_prefix, &subtree_id) != 0)
                return -1;
